		...
		if((isset($item->link) && strpos($item->link,'http://www.facebook.com/events/') !== false) || get_option('ik_fb_show_only_events')){
			...
			if($event_id){
				...
				if(isset($event_data->name)){
					//div
					$replace = '<div class="row fbpost">';

					$replace .= '<div class="col-xs-12 col-sm-12 col-md-2 col-lg-2">';
					//event link


					//event image					
					$replace .= '<a href="http://facebook.com/events/'.urlencode($event_id).'" target="_blank"><img class="ikfb_event_image" src="' . $event_image . '" alt="Event Image"/></a>';		
					...
					$replace .= '<div class="col-xs-12 col-sm-12 col-md-10 col-lg-10">';
					//event name
					$replace .= '<p class="ikfb_event_title">' . $event_data->name . '</p>';
